plan profile::my_app::deploy(
  ...
  unless $force {
    $conns = run_task('profile::my_app::lb', $lb_server,
  ...
  $old_versions = run_task('profile::my_app::install', [$app_servers, $db_server],
    "Install ${version} of the application",
    version => $version
  ).map |$r| { $r['previous_version'] }

  run_task('profile::my_app::migrate', $db_server)
  ...
  without_default_logging() || {
    ...
    get_targets($app_servers).each |$server| {
      ...
      $stats = run_task('profile::my_app::lb', $lb_server,
        action => 'stats',
        backend => $instance,
        server => $server.name,
        _catch_errors => $force
      ...
      run_task('profile::my_app::deploy', [$server],
